...
**One shared Tracker, one poll for every monitor (#54).** The bar exists once per monitor, but Tracker is a single shared instance — one status poll serves every screen instead of one per screen. The 5s/2s cadence above follows whichever monitor's panel is actually open (`panelOpen` is `true` when **any** bar's panel is open), and the poll only stops when **every** bar is hidden. Starting or stopping a Connection from one monitor's panel shows the same busy state and row error on every other monitor, since every panel now reads the same Tracker.
