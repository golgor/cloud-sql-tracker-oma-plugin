...
**Doctor-on-open.** Opening the panel runs `cloud-sql-tracker doctor --json` once (not a continuous poll). From request until doctor settles, the switchboard stays hidden (`doctor_failed` with "Checking setup…"), so a faster Status poll cannot flash the connection list. Hard fail keeps full-body Degraded; pass reveals the list. Status poll stays 5s closed / 2s open while the bar is visible, and stops while the bar is hidden (`omarchy-toggle-bar`) — a hidden bar has no reader for the count. Revealing the bar refreshes at once (#52).
