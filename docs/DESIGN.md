...
| `schema` | Status document `version` ≠ 1. Or the document breaks contract shape — for example, `connections` is not an array. Upgrade plugin or CLI together. A shape break usually means the control plane is broken |
...
**Doctor-on-open.** Opening the panel runs `cloud-sql-tracker doctor --json` once (not a continuous poll). From request until doctor settles, the switchboard stays hidden (`doctor_failed` with "Checking setup…"), so a faster Status poll cannot flash the connection list. Hard fail keeps full-body Degraded; pass reveals the list. Status poll stays 5s closed / 2s open while the bar is visible or the panel is open, and stops launching new polls only when the bar is hidden (`omarchy-toggle-bar`) **and** the panel is closed — a hidden bar with a closed panel has no reader for the count; a hidden bar with the panel still open (the shell parks the bar without closing it) keeps the 2s cadence so its rows keep advancing. Revealing the bar requests a refresh at once — the count repaints once the CLI answers (#52).

**One shared Tracker, one poll for every monitor (#54).** The bar exists once per monitor, but Tracker is a single shared instance — one status poll serves every screen instead of one per screen. The 5s/2s cadence above follows whichever monitor's panel is actually open (`panelOpen` is `true` when **any** bar's panel is open), and the poll only fully stops when **every** bar is hidden **and every** panel is closed (the `barVisible || panelOpen` gate, #52). Starting or stopping a Connection from one monitor's panel shows the same busy state and row error on every other monitor, since every panel now reads the same Tracker. Opening a second monitor's panel while another is already open reuses that settled preflight rather than re-running it — doctor still runs once per panel *open*, not once per monitor.
